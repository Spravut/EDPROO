...
class UserRegisterForm(UserCreationForm):
    """
    Кастомная форма регистрации с дополнительным полем email и выбором роли.
    ...
    role = forms.ChoiceField(
        ...
        help_text='Выберите, как вы будете использовать платформу',
    ...
    def save(self, commit=True):
        """Сохраняет пользователя и создает профиль с выбранной ролью"""
        user = super().save(commit=False)
        if commit:
            user.save()
            # Создаем профиль с выбранной ролью (используем get_or_create для избежания дублирования)
            ...
            # Если профиль уже существовал (создан сигналом), обновляем роль
            if not created:
                profile.role = role
                profile.save()
        return user
...
class CourseRecommendationForm(forms.Form):
    """
    Форма для подбора курсов на основе ответов пользователя.
    Содержит вопросы, которые помогают лучше понять интересы и потребности пользователя.
    """
    ...
    # Вопрос 1: Интерес к программированию
    coding_interest = forms.ChoiceField(
        label='Насколько вам нравится изучать программирование и код?',
        choices=INTEREST_SCALE,
        widget=forms.RadioSelect(attrs={'class': 'form-check-input'}),
        help_text='Оцените ваш интерес к программированию'
    )
    
    # Вопрос 2: Интерес к дизайну
    design_interest = forms.ChoiceField(
        label='Насколько вы хотите улучшить дизайн своих проектов?',
        choices=INTEREST_SCALE,
        widget=forms.RadioSelect(attrs={'class': 'form-check-input'}),
        help_text='Оцените ваш интерес к дизайну'
    )
    
    # Вопрос 3: Веб-разработка
    web_development = forms.ChoiceField(
        label='Хотите ли вы создавать веб-приложения и сайты?',
        choices=INTEREST_SCALE,
        widget=forms.RadioSelect(attrs={'class': 'form-check-input'}),
        help_text='Оцените ваш интерес к веб-разработке'
    )
    
    # Вопрос 4: Мобильная разработка
    mobile_development = forms.ChoiceField(
        label='Интересует ли вас разработка мобильных приложений?',
        choices=INTEREST_SCALE,
        widget=forms.RadioSelect(attrs={'class': 'form-check-input'}),
        help_text='Оцените ваш интерес к мобильной разработке'
    )
    
    # Вопрос 5: Базы данных
    database_interest = forms.ChoiceField(
        label='Хотите ли вы изучать базы данных и работу с данными?',
        choices=INTEREST_SCALE,
        widget=forms.RadioSelect(attrs={'class': 'form-check-input'}),
        help_text='Оцените ваш интерес к базам данных'
    )
    
    # Вопрос 6: Машинное обучение
    ml_interest = forms.ChoiceField(
        label='Интересует ли вас машинное обучение и искусственный интеллект?',
        choices=INTEREST_SCALE,
        widget=forms.RadioSelect(attrs={'class': 'form-check-input'}),
        help_text='Оцените ваш интерес к машинному обучению'
    )
    
    # Вопрос 7: Работа в команде
    TEAM_CHOICES = [
        (1, 'Предпочитаю работать один'),
        (2, 'Скорее один, но могу в команде'),
        (3, 'Без разницы'),
        (4, 'Скорее в команде, но могу один'),
        (5, 'Обязательно в команде'),
    ]
    team_work = forms.ChoiceField(
        label='Как вы предпочитаете работать?',
        choices=TEAM_CHOICES,
        widget=forms.RadioSelect(attrs={'class': 'form-check-input'}),
        help_text='Выберите ваш предпочтительный стиль работы'
    )
    
    # Вопрос 8: Формат обучения
    ...
    learning_format = forms.ChoiceField(
        label='Какой формат обучения вам больше подходит?',
        choices=LEARNING_FORMAT_CHOICES,
        widget=forms.RadioSelect(attrs={'class': 'form-check-input'}),
        help_text='Выберите предпочтительный формат обучения'
    )
    
    # Вопрос 9: Время на обучение
    TIME_CHOICES = [
        ('1-3', '1-3 часа в неделю'),
        ('4-7', '4-7 часов в неделю'),
        ('8-15', '8-15 часов в неделю'),
        ('16+', 'Более 16 часов в неделю'),
    ]
    study_time = forms.ChoiceField(
        label='Сколько времени в неделю вы готовы уделять обучению?',
        choices=TIME_CHOICES,
        widget=forms.RadioSelect(attrs={'class': 'form-check-input'}),
        help_text='Выберите количество часов, которое вы готовы тратить на обучение'
    )
    
    # Вопрос 10: Теория vs Практика
    THEORY_PRACTICE_CHOICES = [
        ('theory', 'Больше теории'),
        ('practice', 'Больше практики'),
        ('balanced', 'Сбалансированно'),
    ]
    theory_practice = forms.ChoiceField(
        label='Что для вас важнее?',
        choices=THEORY_PRACTICE_CHOICES,
        widget=forms.RadioSelect(attrs={'class': 'form-check-input'}),
        help_text='Выберите предпочтительный баланс теории и практики'
    )
    
    # Вопрос 11: Уровень
    level = forms.ChoiceField(
        label='Ваш текущий уровень знаний',
        choices=Course.LEVEL_CHOICES,
        widget=forms.Select(attrs={'class': 'form-select'}),
        help_text='Выберите ваш текущий уровень подготовки'
    )
    
    # Вопрос 12: Бесплатные курсы
    free_only = forms.BooleanField(
        label='Показывать только бесплатные курсы',
        required=False,
        widget=forms.CheckboxInput(attrs={'class': 'form-check-input'}),
        help_text='Отметьте, если хотите видеть только бесплатные курсы'
    )
